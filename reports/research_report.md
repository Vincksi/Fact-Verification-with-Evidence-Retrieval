...
Efficient deployment on CPU hardware relies on indexing and quantization. Malkov & Yashunin (2018) pioneered **HNSW** for fast approximate nearest neighbor search, which achieves logarithmic search complexity compared to the linear search of traditional flat indices [9]. Model quantization, as implemented in the **ONNX Runtime** [10], allows for 8-bit integer inference, which significantly boosts throughput on CPU instructions sets like AVX-512 [11][12].
...
2.  **Dense Path**: 384-dimensional embeddings generated by `all-MiniLM-L6-v2`. We use a FAISS **HNSW** index with 32 neighbors per node and a construction efficiency parameter of 40 for accuracy.
3.  **Fusion (RRF)**: We combine the ranks from both indices using a smoothing constant of 60. This fusion ensures that if a document is 1st in BM25 but 100th in Dense, it still remains in the top-k results for the verifier.
...
- **Dynamic Attention**: High-order attention coefficients are calculated by concatenating query, key, and edge features before applying non-linear transformations. This allows the model to learn that "Claim -> Evidence" links have different logical weight than "Sentence -> Entity" links.
...
- **Hyperparameters**: Optimizer: AdamW, Learning Rate: 0.0001, Dropout: 0.1, Heads: 4, Hidden Dim: 256.
...
We performed a **one-tailed paired t-test** on a sample of 300 claims for Recall@10 (BM25 vs. Hybrid). The results showed extremely high statistical significance with a p-value less than 0.001. For verification accuracy comparisons between GAT and GATv2, we obtained a p-value of 0.042, also confirming significant improvement at the 5% level. Both results indicate that our architectural changes provide statistically significant improvements.
